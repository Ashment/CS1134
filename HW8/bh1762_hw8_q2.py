...
def create_complete_bst(n):
	...
	add_items(bst, 1, n);								# Θ(n)
	return bst;											# Runtime: Θ(n)

def add_items(bTree, low, high):
	curMid = int((low+high)/2);
	bTree.subtree_insert(curMid);						# Θ(n)
	if(low != high):
		add_items(bTree, low, curMid-1);				# Θ(n)
		add_items(bTree, curMid+1, high);				# Θ(n)
	return												# Runtime: Θ(n)


def DEBUG():
	t = create_complete_bst(7);
	for e in t:
		print(e);
