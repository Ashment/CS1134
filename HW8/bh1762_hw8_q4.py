...
def find_min_abs_difference(bst):
	return find_sub_min_diff(bst.root)[0];


def find_sub_min_diff(curRoot):
	if(curRoot.left is None and curRoot.right is None):
		return (curRoot.item.key, curRoot.item.key, curRoot.item.key);
	else:
		curVal = curRoot.item.key;

		subDiffArr = [];

		#Navigate Left Subtree and find difference.
		if(curRoot.left is None):
			sMin = curRoot.item.key;
		else:
			nextLeft = find_sub_min_diff(curRoot.left);
			sMin = nextLeft[1];
			leftMax = nextLeft[2];
			subDiffArr.append(min(nextLeft[0], curVal - leftMax));

		if(curRoot.right is None):
			sMax = curRoot.item.key;
		else:
			nextRight = find_sub_min_diff(curRoot.right);
			sMax = nextRight[2];
			rightMin = nextRight[1];
			subDiffArr.append(min(nextRight[0], rightMin - curVal));

	return (min(subDiffArr), sMin, sMax);
